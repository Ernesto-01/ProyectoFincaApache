...
public class ControlDetalleEnvio extends Conexion{
    ...
    public List mostrarDetalleEnvio(int buscar) throws Exception {
        ...
        try {
            this.conectar();
            if (buscar>0) {
                sql="CALL sp_detalleenvio(11,"+buscar+",0,'',0,'')";
            } else {
                sql="CALL sp_detalleenvio(4,0,0,'',0,'')";
            }
    ...
    public void mostrarEnvios(JComboBox cmb) {
        ...
        try {
            this.conectar();
            String sql = "CALL sp_detalleenvio(5,0,0,'',0,'')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();
            while (res.next()) {
    ...
    public int mostrarIDEnvios(String nombre) throws Exception {
        ...
        try {
            this.conectar();
            String sql="CALL sp_detalleenvio(6,0,0,'"+nombre+"',0,'')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();            
    ...
    public String mostrarNombreEnvio(int id) throws Exception {
        ...
        try {
            this.conectar();
            String sql="CALL sp_detalleenvio(7,0,"+id+",'',0,'')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();            
            DetalleEnvio de = new DetalleEnvio();
            while(res.next()) {
    ...
    public void mostrarInsumo(JComboBox cmb) {
        ...
        try {
            this.conectar();
            String sql = "CALL sp_detalleenvio(8,0,0,'',0,'')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();
            while (res.next()) {
                cmb.addItem(res.getString("nombre"));
    ...
    public int mostrarIDInsumo(String nombre) throws Exception {
        ...
        try {
            this.conectar();
            String sql="CALL sp_detalleenvio(9,0,0,'',0,'"+nombre+"')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();            
            DetalleEnvio de = new DetalleEnvio();
            while(res.next()) {
    ...
    public String mostrarNombreInsumos(int id) throws Exception {
        ...
        try {
            this.conectar();
            String sql="CALL sp_detalleenvio(10,0,0,'',"+id+",'')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();            
            DetalleEnvio de = new DetalleEnvio();
            while(res.next()) {
