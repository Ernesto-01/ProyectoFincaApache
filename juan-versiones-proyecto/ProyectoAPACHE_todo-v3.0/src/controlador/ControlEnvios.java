...
public class ControlEnvios extends Conexion {
     public void mantenimientoEnvio(int accion,Envios env) throws Exception {
        try {
            ...
            pre.setInt(5, env.getCantidadEnvios());
            pre.setString(6, env.getNombreReceptor());
            pre.setInt(7, env.getIdFinca());
            pre.setString(8, "");
            pre.executeUpdate();
        } catch(Exception e) {
           throw e; 
    ...
    public List mostrarEnvio(String buscar) throws Exception {
        ...
        try {
            this.conectar();
            if (!"".equals(buscar)) {
                sql = "CALL sp_envios(11,0,'"+buscar+"','',0,'',0,'',0,'')";
            } else {
                sql = "CALL sp_envios(4,0,'','',0,'',0,'')";
            }
            ...
            while(res.next()){
                ...
                env.setCantidadEnvios(res.getInt("cantidadEnvios"));
    ...
    public void mostrarFinca(JComboBox cmb) {
        ...
        try {
            this.conectar();
            String sql = "CALL sp_envios(5,0,'','',0,'',0,'',0,'')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();
            while (res.next()) {
    ...
    public int mostrarIDFinca(String nombre) throws Exception {
        ...
        try {
            this.conectar();
            String sql="CALL sp_envios(6,0,'','',0,'',0,'"+nombre+"',0,'')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();            
    ...
    public String mostrarNombreFinca(int id) throws Exception {
        ...
        try {
            this.conectar();
            String sql="CALL sp_envios(7,0,'','',0,'',"+id+",'',0,'')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();            
            Envios en = new Envios();
            while(res.next()) {
    ...
    public void mostrarInsumo(JComboBox cmb) {
        ...
        try {
            this.conectar();
            String sql = "CALL sp_envios(8,0,'','',0,'',0,'',0,'')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();
            while (res.next()) {
                cmb.addItem(res.getString("nombre"));
    ...
    public int mostrarIDInsumo(String nombre) throws Exception {
        ...
        try {
            this.conectar();
            String sql="CALL sp_envios(9,0,'','',0,'',0,'',0,'"+nombre+"')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();            
            Envios en = new Envios();
            while(res.next()) {
                en.setIdInsumo(res.getInt("idInsumo"));
            }            
            listarinsumo=en.getIdInsumo();
        } catch(Exception e) {
            throw e;
        } finally {
            this.desconectar();
    ...
    public String mostrarNombreInsumos(int id) throws Exception {
        ...
        try {
            this.conectar();
            String sql="CALL sp_envios(10,0,'','',0,'',0,'',"+id+",'')";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();            
            Envios en = new Envios();
            while(res.next()) {
                en.setNombreInsumo(res.getString("nombre"));
            }            
            listarenvio=en.getNombreInsumo();
        } catch(Exception e) {
            throw e;
        } finally {
            this.desconectar();
