...
public class ControlInsumo extends Conexion {
     public void mantenimientoInsumo(int accion,Insumo isu) throws Exception {
        try {
            ...
            pre.setDouble(6, isu.getPrecioInsumo());
            pre.setInt(7, isu.getIdTipoInsumo());
            pre.executeUpdate();
        } catch(Exception e) {
           throw e; 
        } finally {
    ...
    public List mostrarInsumo(String buscar) throws Exception {
        ...
        try {
            this.conectar();
            if (!"".equals(buscar)) {
                sql="CALL sp_insumos(6,0,'"+buscar+"','',0,0,0)";
            } else {
                sql="CALL sp_insumos(4,0,'','',0,0,0)";
            }
            ...
            while(res.next()){
                ...
                isu.setPrecioInsumo(res.getDouble("precioInsumo"));
    ...
    public void mostrarTipoInsumos(JComboBox cmb) {
        ...
        try {
            this.conectar();
            String sql = "CALL sp_insumos(5,0,'','',0,0,0)";
            PreparedStatement pre = this.getCon().prepareCall(sql);
            res=pre.executeQuery();
            while (res.next()) {
