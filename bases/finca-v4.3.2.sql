...
insert  into `tbl_detalleenvio`(`idDetalleEnvio`,`cantidadEnvios`,`idEnvios`,`idInsumo`) values (1,5,1,1);
...
insert  into `tbl_envios`(`idEnvios`,`nombre`,`fechaEnvio`,`nombreReceptor`,`idFinca`) values (1,'envio 1','2018-05-31','nombre 1',1);
...
insert  into `tbl_inventario`(`idInventario`,`cantidad`,`precio`,`estado`,`idFinca`,`idProveedor`,`idInsumo`) values (1,5,50,1,1,1,1);
...
BEGIN
	...
	ELSEIF Reporte = 5 THEN
		BEGIN
			select i.idInsumo,i.nombre,i.medida,i.cantidad,t.nombre from tbl_insumos i 
			join tbl_tipoinsumo t on i.idTipoInsumo = t.idTipoInsumo;
		END;
	ELSEIF Reporte = 6 THEN
		BEGIN
			select e.idEnvios,e.nombre,e.fechaEnvio,e.nombreReceptor,f.nombre from tbl_envios e
			join tbl_finca f on e.idFinca = f.idFinca;
		END;
	ELSEIF Reporte = 7 THEN
		BEGIN
			select n.idInventario,n.cantidad,n.precio,f.nombre,p.nombre,i.nombre from tbl_inventario n
