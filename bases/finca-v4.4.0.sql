...
insert  into `tbl_detalleenvio`(`idDetalleEnvio`,`idEnvios`,`idInsumo`) values (1,1,1);
...
insert  into `tbl_envios`(`idEnvios`,`nombre`,`fechaEnvio`,`cantidadEnvios`,`nombreReceptor`,`idFinca`) values (1,'envio 1','2018-05-31',0,'nombre 1',1);
...
insert  into `tbl_insumos`(`idInsumo`,`nombre`,`medida`,`cantidad`,`precioInsumo`,`idTipoInsumo`) values (1,'insumo 1','5',5,0,1);
...
insert  into `tbl_insumosutilizados`(`idInsumoUtilizados`,`cantidadUtilizada`,`idInsumo`,`idTareaDiaria`) values (1,5,1,1),(2,6,1,1);
...
insert  into `tbl_inventario`(`idInventario`,`cantidad`,`precioTotal`,`estado`,`idFinca`,`idProveedor`,`idInsumo`) values (1,5,50,1,1,1,1);
...
insert  into `tbl_jornaleros`(`idJornaleros`,`nombre`,`apellido`,`edad`,`horario`,`estado`,`direccion`,`idPagos`) values (1,'jornalero','apellido',20,'5',1,'direccion',1);
...
insert  into `tbl_tareadiaria`(`idTareaDiaria`,`fecha`,`estado`,`jornada`,`porcentajeRealizado`,`idTarea`,`idTablones`,`idJornalero`) values (1,'2018-05-31',1,'fgh','50%',1,1,1);
...
BEGIN
    IF Accion = 1 THEN
	BEGIN
		INSERT INTO tbl_detalleenvio(idEnvios,idInsumo) VALUES(vIDENVIOS,vIDINSUMO);
		SELECT 1 AS 'Mensaje';
	END;
    ELSEIF Accion = 2 THEN
	BEGIN
		UPDATE tbl_detalleenvio SET idEnvios=vIDENVIOS,idInsumo=vIDINSUMO WHERE idDetalleEnvio=vIDDETALLEENVIO;
		SELECT 1 AS 'Mensaje';
	END;
...
SET ID=0;
    IF Accion = 1 THEN
	BEGIN
		insert into tbl_envios(nombre,fechaEnvio,cantidadEnvios,nombreReceptor,idFinca) values(vNOMBRE,DATE_FORMAT(vFECHAENVIO,'%Y-%m-%d'),vCANTIDADENVIOS,vNOMBRERECEPTOR,vIDFINCA);
		set ID = last_insert_id();
		insert into tbl_detalleenvio(idEnvios,idInsumo) values(ID,vIDINSUMO);
		SELECT 1 AS 'Mensaje';
	END;
    ELSEIF Accion = 2 THEN
	BEGIN
		update tbl_envios set nombre=vNOMBRE,fechaEnvio=DATE_FORMAT(vFECHAENVIO,'%Y-%m-%d'),cantidadEnvios=vCANTIDADENVIOS,nombreReceptor=vNOMBRERECEPTOR,idFinca=vIDFINCA where idEnvios=vIDENVIOS;
		IF vIDINSUMO > 0 THEN 
			BEGIN
				UPDATE tbl_detalleenvio set idInsumo=vIDINSUMO WHERE idEnvios=vIDENVIOS;
			END;
		END IF;
		SELECT 1 AS 'Mensaje';
	END;
    ...
    ELSEIF Accion = 4 THEN
	BEGIN
		SELECT idEnvios,nombre,date_format(fechaEnvio,'%d-%m-%Y') as fechaEnvio,cantidadEnvios,nombreReceptor,idFinca FROM tbl_envios;
	END;
    ELSEIF Accion = 5 THEN
	BEGIN
    ...
    ELSEIF Accion = 11 THEN
	BEGIN
		SELECT idEnvios,nombre,DATE_FORMAT(fechaEnvio,'%d-%m-%Y') AS fechaEnvio,cantidadEnvios,nombreReceptor,idFinca FROM tbl_envios WHERE nombre like concat("%",vNOMBRE,"%");
	END;
    END IF;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insumos`(
	...
	vMEDIDA VARCHAR(50),
	vCANTIDAD DOUBLE,
	vPRECIOINSUMO DOUBLE,
	vIDTIPOINSUMO INT
    )
BEGIN
    IF Accion = 1 THEN
	BEGIN
		INSERT INTO tbl_insumos(nombre,medida,cantidad,precioInsumo,idTipoInsumo) VALUES(vNOMBRE,vMEDIDA,vCANTIDAD,vPRECIOINSUMO,vIDTIPOINSUMO);
		SELECT 1 AS 'Mensaje';
	END;
    ELSEIF Accion = 2 THEN
	BEGIN
		UPDATE tbl_insumos SET nombre=vNOMBRE,medida=vMEDIDA,cantidad=vCANTIDAD,precioInsumo=vPRECIOINSUMO,idTipoInsumo=vIDTIPOINSUMO WHERE idInsumo=vIDINSUMO;
		SELECT 1 AS 'Mensaje';
	END;
    ELSEIF Accion = 3 THEN
	BEGIN
...
BEGIN
    IF Accion = 1 THEN
	BEGIN
		INSERT INTO tbl_inventario(cantidad,precioTotal,estado,idFinca,idProveedor,idInsumo) VALUES(vCANTIDAD,vPRECIO,vESTADO,vIDFINCA,vIDPROVEEDOR,vIDINSUMO);
		SELECT 1 AS 'Mensaje';
	END;
    ELSEIF Accion = 2 THEN
	BEGIN
		UPDATE tbl_inventario SET cantidad=vCANTIDAD,precioTotal=vPRECIO,estado=vESTADO,idFinca=vIDFINCA,idProveedor=vIDPROVEEDOR,idInsumo=vIDINSUMO WHERE idInventario=vIDINVENTARIO;
		SELECT 1 AS 'Mensaje';
	END;
    ELSEIF Accion = 3 THEN
	BEGIN
...
/* Procedure structure for procedure `sp_reportes` */

/*!50003 DROP PROCEDURE IF EXISTS  `sp_reportes` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_reportes`(Reporte int)
BEGIN
	if Reporte = 1 then
		begin
			select j.idJornaleros,j.nombre,j.apellido,j.edad,j.horario,j.direccion,p.salarioDiario from tbl_jornaleros j
			join tbl_pagos p on j.idPagos = p.idPagos where j.estado=1;
		end;
	elseIF Reporte = 2 THEN
		BEGIN
			select d.idTareaDiaria,d.fecha,d.jornada,d.porcentajeRealizado,a.nombre,t.nombre,j.nombre from tbl_tareadiaria d 
			join tbl_tareas a on d.idTarea = a.idTarea
			join tbl_tablones t on d.idTablones = t.idTablones
			join tbl_jornaleros j on d.idJornalero = j.idJornaleros where d.estado=1;
		END;
	ELSEIF Reporte = 3 THEN
		BEGIN
			select d.idDetalleEnvio,d.cantidadEnvios,e.idEnvios,i.idInsumo from tbl_detalleenvio d
			join tbl_envios e on d.idEnvios = e.idEnvios
			join tbl_insumos i on d.idInsumo = i.idInsumo;
		END;
	ELSEIF Reporte = 4 THEN
		BEGIN
			select u.idInsumoUtilizados,u.cantidadUtilizada,i.idInsumo,d.idTareaDiaria from tbl_insumosutilizados u
			join tbl_insumos i on u.idInsumo = i.idInsumo
			join tbl_tareadiaria d on u.idTareaDiaria = d.idTareaDiaria;
		END;
	ELSEIF Reporte = 5 THEN
		BEGIN
			select i.idInsumo,i.nombre,i.medida,i.cantidad,i.precioInsumo,t.nombre from tbl_insumos i 
			join tbl_tipoinsumo t on i.idTipoInsumo = t.idTipoInsumo;
		END;
	ELSEIF Reporte = 6 THEN
		BEGIN
			select e.idEnvios,e.nombre,e.fechaEnvio,e.cantidadEnvios,e.nombreReceptor,f.nombre from tbl_envios e
			join tbl_finca f on e.idFinca = f.idFinca;
		END;
	ELSEIF Reporte = 7 THEN
		BEGIN
			select n.idInventario,n.cantidad,n.precioTotal,f.nombre,p.nombre,i.nombre from tbl_inventario n
			join tbl_finca f on n.idFinca = f.idFinca
			join tbl_proveedor p on n.idProveedor = p.idProveedor
			join tbl_insumos i on n.idInsumo = i.idInsumo where n.estado=1;
		END;
	end if;
    END */$$
DELIMITER ;
